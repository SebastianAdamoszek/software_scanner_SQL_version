...
def show_menu(search, publisher, missing):
    ...
    console.print(Panel(status, title="STATUS", border_style="yellow"))
    ...
    table.add_column("Opcja", width=6, justify="center")
    ...
    table.add_row("[cyan]4[/cyan]", f"Pokaż tylko programy bez wersji: {'TAK' if missing else 'NIE'}")
    table.add_row("[yellow]5[/yellow]", "Resetuj filtry")
    table.add_row("[red]6[/red]", "Wyjście")

    console.print(table)

    console.print("\n[dim]Wpisz numer opcji i Enter[/dim]")
...
def show_system_status(system_info):
    ...
    table.add_row("Administrator", format_bool_pl(system_info.get("is_admin", False)))

    system = system_info.get("system", "N/A")
    release = system_info.get("release", "")
    machine = system_info.get("machine", "")

    table.add_row("System", f"{system} {release} ({machine})")

    border_style = "green" if system_info.get("is_admin") else "yellow"
...
def show_no_filter_results(total_before_filters):
    console.print("[yellow]Brak wyników po filtrach.[/yellow]")
    console.print(f"Przed filtrem: [bold]{total_before_filters}[/bold]")
...
def show_summary(summary):
    ...
    table.add_row("Programy", str(summary.get("program_count", 0)))
    table.add_row("Przed filtrem", str(summary.get("total_before_filters", 0)))
    table.add_row("Producenci", str(summary.get("publisher_count", 0)))
    table.add_row("Bez wersji", str(summary.get("missing_version_count", 0)))
    table.add_row("Bez producenta", str(summary.get("missing_publisher_count", 0)))
...
def show_filters(filter_info):
    if not filter_info.get("active"):
        console.print("[dim]Brak aktywnych filtrów[/dim]")
        return

    table = Table(title="Filtry", border_style="magenta")
    table.add_column("Filtr")
    table.add_column("Wartość")

    if filter_info.get("search"):
        table.add_row("Nazwa", filter_info["search"])

    if filter_info.get("publisher"):
        table.add_row("Producent", filter_info["publisher"])

    if filter_info.get("missing_version_only"):
        table.add_row("Bez wersji", "TAK")
...
def show_top_publishers(summary, limit=5):
    top = summary.get("top_publishers", [])

    if not top:
        console.print("[dim]Brak danych o producentach[/dim]")
        return

    table = Table(title="LISTA PRODUCENTÓW", border_style="green")
...
def show_programs_table(programs, limit=30):
    ...
    for i, p in enumerate(programs[:limit], start=1):
        table.add_row(
            str(i),
            p.get("name", "-"),
            p.get("version") or "-",
    ...
    if len(programs) > limit:
        console.print(f"[dim]Pokazano {limit} z {len(programs)}[/dim]")
